...
difficulty = st.radio(
    "난이도를 선택하세요",
    ["쉬움 (Easy)", "보통 (Normal)", "어려움 (Hard - 15초 생존 미션!)"],
    index=2,
    horizontal=True
)

# 난이도별 게임 변수 설정 (target_time: 목표 시간, None은 무제한)
difficulty_settings = {
    "쉬움 (Easy)": {"spawn_rate": 400, "base_speed": 1.5, "speed_inc": 0.05, "key": "easy", "target_time": None},
    "보통 (Normal)": {"spawn_rate": 250, "base_speed": 2.2, "speed_inc": 0.1, "key": "normal", "target_time": None},
    "어려움 (Hard - 15초 생존 미션!)": {"spawn_rate": 150, "base_speed": 3.0, "speed_inc": 0.2, "key": "hard", "target_time": 15.0}
}

cfg = difficulty_settings[difficulty]
target_time_js = "null" if cfg["target_time"] is None else cfg["target_time"]
...
<body>
    ...
    <script>
        ...
        const targetTime = {target_time_js}; // 어려움 모드 15초 목표
